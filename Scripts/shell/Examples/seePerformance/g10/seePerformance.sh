...
echo "####################################################################"
echo "# DpiProcessor CPU UTILIZATION (%)"
echo "####################################################################"
...
echo "####################################################################"
echo "# PER-BLADE PACKETS-PER-SECOND (CRX)"
echo "####################################################################"
...
##########################################################################
## Statistic: "pdusPerSecond" (Packets-Per-Second) AVG
##########################################################################
cat ~/myFile | perl -we '
$input=<STDIN>;
@fields = split ",", $input;
...
echo "####################################################################"
echo "# TOTAL PACKETS-PER-SECOND (CRX)"
echo "####################################################################"
...
echo "####################################################################"
echo "# Per Blade: Total XDR's Generated"
echo "####################################################################"
...
echo "####################################################################"
echo "# Per Blade: Total XDR's Sent"
echo "####################################################################"
...
echo "####################################################################"
echo "# Per Blade: Total XDR's Dropped"
echo "####################################################################"
...
echo "####################################################################"
echo "# Per Blade: XDR's Per Second"
echo "####################################################################"
...
echo "####################################################################"
echo "# Per Blade: XDR's Dropped Per Second"
echo "####################################################################"
...
echo "####################################################################"
echo "# TOTAL XDR's Dropped Per Sec"
echo "####################################################################"
...
echo "####################################################################"
echo "# TrafficProcessor Residential Memory Utilization (GigaBytes)"
echo "####################################################################"
...
##########################################################################
## Statistic: "vmRSS" (Residential Memory Utilization) AVG
##########################################################################
cat ~/myFile | perl -we '
$input=<STDIN>;
@fields = split ",", $input;
...
if (! (defined($vmRSS) && defined ($period) && defined($time) && defined($blade)))
{
    die("Could not find some field");
}
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime )
    {
        foreach $key (keys(%currentVmRSS))
        {
            my $rate = $currentVmRSS{$key}/$currentPeriod;

            if (! exists($minVmRSS{$key}) || ($rate < $minVmRSS{$key}))
            {
                $minVmRSS{$key} = $rate;
            }
            if (! exists($maxVmRSS{$key}) || ($rate > $maxVmRSS{$key}))
            {
                $maxVmRSS{$key} = $rate;
            }
            $totalVmRSS{$key} += $currentVmRSS{$key};
            $currentVmRSS{$key} = 0;
        }
        $totalPeriod += $currentPeriod;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentVmRSS{$fields[$blade]} += $fields[$vmRSS]*$currentPeriod;
}
...
foreach $key (keys(%currentVmRSS))
{
    ...
    printf "$key Avg:%.3f\n", ((($totalVmRSS{$key}/$totalPeriod/1024)/1024)/1000);
     #printf "$key Avg:%.1f\n", ((((($totalVmRSS{key})/$totalPeriod)/1024)/1024)/1000);

    if (exists $minVmRSS{$key})
    {
        printf "$key Min:%.3f\n", ((($minVmRSS{$key})/1024)/1024)/1000 ;
        printf "$key Max:%.3f\n", ((($maxVmRSS{$key})/1024)/1024)/1000 ;
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) Flow Record Alloc Failures:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) FSB Alloc Failures:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) FSB Out Of Sync:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) FSB Search Failures:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) Hash Table Insert Fails:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) Hash Table Remove Fails:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) Timer Failures:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) Tunnel Update Fails:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
...
rm -rf ~/myFile
echo "#################################################################################################"
echo "# Tunnel FSPP (Flow State Packet Processing) Total Packets:"
echo "#################################################################################################"
cat `ls -rt ./IicTunnelFsppStats* | tail -1`| perl -e '
$input=<STDIN>;
print $input;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentassocAllocFailures/$currentPeriod;

        $totalPeriod += $currentPeriod;
        $totalassocAllocFailures += $currentassocAllocFailures;
        $currentassocAllocFailures = 0;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentassocAllocFailures += $fields[$assocAllocFailures];
}
my $drops = $currentassocAllocFailures/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentassocTblEntries/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentassocTblInsertFails/$currentPeriod;

        $totalPeriod += $currentPeriod;
        $totalassocTblInsertFails += $currentassocTblInsertFails;
        $currentassocTblInsertFails = 0;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentassocTblInsertFails += $fields[$assocTblInsertFails];
}
my $drops = $currentassocTblInsertFails/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentbitmapAllocFailures/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentbundledPkts/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentchunksLostReassembly/$currentPeriod;

        $totalPeriod += $currentPeriod;
        $totalchunksLostReassembly += $currentchunksLostReassembly;
        $currentchunksLostReassembly = 0;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentchunksLostReassembly += $fields[$chunksLostReassembly];
}
my $drops = $currentchunksLostReassembly/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentcloneFailed/$currentPeriod;

        $totalPeriod += $currentPeriod;
        $totalcloneFailed += $currentcloneFailed;
        $currentcloneFailed = 0;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentcloneFailed += $fields[$cloneFailed];
}
my $drops = $currentcloneFailed/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentdisregardedPkts/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentdroppedAddresses/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentdroppedDetection/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentdroppedPkts/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentfragmentsDropped/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentfragmentsHeld/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentincompleteInits/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentincompleteReassemblies/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentinconsistentAssocs/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentlinkTblEntries/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentlinkTblInsertFails/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentmonitoredPaths/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentoverrunResets/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentoverruns/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentpathsWithNoAssoc/$currentPeriod;

        $totalPeriod += $currentPeriod;
        $totalpathsWithNoAssoc += $currentpathsWithNoAssoc;
        $currentpathsWithNoAssoc = 0;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentpathsWithNoAssoc += $fields[$pathsWithNoAssoc];
}
my $drops = $currentpathsWithNoAssoc/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentprocessedPkts/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentretransmissionResets/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentretransmissios/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentsackOutOfRange/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentseqCreateFailed/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentsequenceErrors/$currentPeriod;

        $totalPeriod += $currentPeriod;
        $totalsequenceErrors += $currentsequenceErrors;
        $currentsequenceErrors = 0;
    }
    $currentTime = $fields[$time];
    $currentPeriod = $fields[$period] / 1000;
    $currentsequenceErrors += $fields[$sequenceErrors];
}
my $drops = $currentsequenceErrors/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentstreamAddFailed/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currenttsnOutOfRange/$currentPeriod;
...
while ($input = <STDIN>)
{
    ...
    if (defined ($currentTime ) && $fields[$time] ne $currentTime)
    {
                my $drops = $currentvtagResets/$currentPeriod;
