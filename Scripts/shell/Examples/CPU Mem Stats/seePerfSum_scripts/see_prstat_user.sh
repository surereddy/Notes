...
echo "$raw_data" | awk 'BEGIN{FS="G"} {if(NR==1) {max=$1;min=$1;sum=0}} {CPU_NoPer[NR]=$1; if (min>CPU_NoPer[NR]){min=CPU_NoPer[NR]};if (max<CPU_NoPer[NR]){max=CPU_NoPer[NR]};sum=sum+CPU_NoPer[NR]} {if(NR==1){ print "First one:", CPU_NoPer[NR]}} END{printf " avg:%0.1f\n max:%0.1f\n min:%0.1f\n sum:%0.1f\n",sum/NR,max,min,sum}';